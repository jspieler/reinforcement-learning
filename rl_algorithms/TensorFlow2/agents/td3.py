...
class TD3:
    def __init__(self, num_actions, num_states, min_action, max_action, lr_actor=1e-3,
                 ...
                 hidden_size=(512,512), batch_size=64, noise_stddev=0.2,
    ...
    def choose_action(self, state, evaluate=False):
        ...
        actions = np.array([1]) * np.clip(sampled_actions, self.min_action, self.max_action)
    ...
    def update(self, state_batch, action_batch, reward_batch, next_state_batch, done_batch):
        ...
        if self.trainstep % self.actor_update_steps == 0:
            ...
            with tf.GradientTape() as tape3:
                new_policy_actions = self.actor(state_batch, self.max_action, training=True)
                critic_value = self.critic(state_batch, new_policy_actions, training=True)
