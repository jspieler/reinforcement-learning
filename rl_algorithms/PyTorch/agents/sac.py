...
class SAC:
    def __init__(self, num_actions, num_states, min_action, max_action, action_space,
                 lr_actor=3e-4, lr_critic=3e-4, lr_alpha=3e-4, gamma=0.99, 
                 buffer_capacity=100000, tau=0.005, alpha=0.2, hidden_size=(512,512), 
                 batch_size=64, update_frequency=2, target_entropy_tuning=True):
        self.gamma = gamma
        self.tau = tau
        ...
        self.actor = SoftActor(num_actions, num_states, action_space, hidden_size).to(device=self.device)
        self.critic = Critic(num_actions+num_states, hidden_size).to(device=self.device)
        self.critic2 = Critic(num_actions+num_states, hidden_size, name='critic2').to(device=self.device)
        self.target_critic = Critic(num_actions+num_states, hidden_size, name='target_critic').to(device=self.device)
        self.target_critic2 = Critic(num_actions+num_states, hidden_size, name='target_critic2').to(device=self.device)
        ...
        if self.target_entropy_tuning is True:
            self.target_entropy = - torch.Tensor([num_actions]).to(self.device)
    ...
    def update(self, state_batch, action_batch, reward_batch, next_state_batch, done_batch):
        ...
        next_state_target_value = torch.min(target_next_state_values, target_next_state_values2) - self.alpha * target_log_pi
